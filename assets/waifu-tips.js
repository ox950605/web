window.live2d_settings = Array(); /*
    ...
    　　　　　 　　　'ｰ'　　!_,.:*********************************************************************************/
        ...
        live2d_settings['modelId'] = 1;                  // 默認模型 ID
        live2d_settings['modelTexturesId'] = 87;         // 預設材質 ID
        ...
        //live2d_settings['waifuSize'] = '400x330';        // 看板娘大小
        //live2d_settings['waifuTipsSize'] = '470x150';    // 提示框大小
        live2d_settings['waifuFontSize'] = '30px';       // 提示框字型
        live2d_settings['waifuToolFont'] = '36px';       // 工具欄字型
...
// 後端介面
live2d_settings['modelAPI'] = '//live2d.fghrsh.net/api/';   // 自建 API 修改這裡
live2d_settings['tipsMessage'] = 'waifu-tips.json';            // 同目錄下可省略路徑
live2d_settings['hitokotoAPI'] = 'hitokoto.cn';                  // 一言 API，可選 'lwl12.com', 'hitokoto.cn', 'jinrishici.com'(古詩詞)

// 預設模型
live2d_settings['modelId'] = 1;            // 預設模型 ID，可在 F12 控制檯找到
live2d_settings['modelTexturesId'] = 53;           // 預設材質 ID，可在 F12 控制檯找到

// 工具欄設定
live2d_settings['showToolMenu'] = true;         // 顯示 工具欄          ，可選 true(真), false(假)
live2d_settings['canCloseLive2d'] = true;         // 顯示 關閉看板娘  按鈕，可選 true(真), false(假)
live2d_settings['canSwitchModel'] = true;         // 顯示 模型切換    按鈕，可選 true(真), false(假)
live2d_settings['canSwitchTextures'] = true;         // 顯示 材質切換    按鈕，可選 true(真), false(假)
live2d_settings['canSwitchHitokoto'] = true;         // 顯示 一言切換    按鈕，可選 true(真), false(假)
live2d_settings['canTakeScreenshot'] = true;         // 顯示 看板娘截圖  按鈕，可選 true(真), false(假)
live2d_settings['canTurnToHomePage'] = true;         // 顯示 返回首頁    按鈕，可選 true(真), false(假)
live2d_settings['canTurnToAboutPage'] = true;         // 顯示 跳轉關於頁  按鈕，可選 true(真), false(假)

// 模型切換模式
live2d_settings['modelStorage'] = true;         // 記錄 ID (重新整理后恢復)，可選 true(真), false(假)
live2d_settings['modelRandMode'] = 'switch';     // 模型切換，可選 'rand'(隨機), 'switch'(順序)
live2d_settings['modelTexturesRandMode'] = 'rand';       // 材質切換，可選 'rand'(隨機), 'switch'(順序)

// 提示訊息選項
live2d_settings['showHitokoto'] = true;         // 顯示一言
live2d_settings['showF12Status'] = true;         // 顯示載入狀態
live2d_settings['showF12Message'] = false;        // 顯示看板娘訊息
live2d_settings['showF12OpenMsg'] = true;         // 顯示控制檯打開提示
live2d_settings['showCopyMessage'] = true;         // 顯示 複製內容 提示
live2d_settings['showWelcomeMessage'] = true;         // 顯示進入面頁歡迎詞

//看板娘樣式設定
live2d_settings['waifuSize'] = '280x250';    // 看板娘大小，例如 '280x250', '600x535'
live2d_settings['waifuTipsSize'] = '250x70';     // 提示框大小，例如 '250x70', '570x150'
live2d_settings['waifuFontSize'] = '12px';       // 提示框字型，例如 '12px', '30px'
live2d_settings['waifuToolFont'] = '14px';       // 工具欄字型，例如 '14px', '36px'
live2d_settings['waifuToolLine'] = '20px';       // 工具欄行高，例如 '20px', '36px'
live2d_settings['waifuToolTop'] = '0px'         // 工具欄頂部邊距，例如 '0px', '-60px'
live2d_settings['waifuMinWidth'] = 'disable';      // 面頁小於 指定寬度 隱藏看板娘，例如 'disable'(禁用), '768px'
live2d_settings['waifuEdgeSide'] = 'left:0';     // 看板娘貼邊方向，例如 'left:0'(靠左 0px), 'right:30'(靠右 30px)
live2d_settings['waifuDraggable'] = 'axis-x';    // 拖拽樣式，例如 'disable'(禁用), 'axis-x'(只能水平拖拽), 'unlimited'(自由拖拽)
live2d_settings['waifuDraggableRevert'] = false;         // 鬆開滑鼠還原拖拽位置，可選 true(真), false(假)

// 其他雜項設定
live2d_settings['l2dVersion'] = '1.4.2';        // 目前版本
live2d_settings['l2dVerDate'] = '2018.11.12'; // 版本更新日期
live2d_settings['homePageUrl'] = 'auto';       // 主頁地址，可選 'auto'(自動), '{URL 網址}'
live2d_settings['aboutPageUrl'] = 'https://github.com/imuncle/live2d';   // 關於頁地址, '{URL 網址}'
live2d_settings['screenshotCaptureName'] = 'live2d.png'; // 看板娘截圖檔名，例如 'live2d.png'
...
function loadModel(modelId, modelTexturesId = 0) {
  ...
  } loadlive2d('live2d', live2d_settings.modelAPI + 'get/?id=' + modelId + '-' + modelTexturesId, (live2d_settings.showF12Status ? console.log('[Status]', 'live2d', '模型', modelId + '-' + modelTexturesId, '載入完成') : null));
}
